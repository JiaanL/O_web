...
func Get_block_data_main() {
	save_file_path := "data/crawled/"

	file_num := "0"
	log_csv := "blockTime_data_" + file_num + ".csv"
	log_headers := []string{
		"blockNum", "blockTime",
	}
	wg := &sync.WaitGroup{}
	...
	if file_num == "0" {
		...
		blockMax = uint64(12700000)
	}

	// 1 ---------------------------
	if file_num == "1" {
		blcokFrom = big.NewInt(12700000)
	...
	for blcokFrom.Uint64() <= blockMax {
		...
		go get_block_data_loop(blcokFrom, blockTo, wg, c)
		blcokFrom = blockTo
...
func get_block_data_loop(blockFrom *big.Int, blockTo *big.Int, wg *sync.WaitGroup, c chan [][2]string) {
