...
import (
	...
	DecodeByAbi "eth_crawler/pkg/decode_by_abi"
...
func Get_tx_to_target_contract_main() {

	// blcokFrom := big.NewInt(12466826)
	blcokFrom := big.NewInt(13426826)
	blockMax := uint64(13838875)
	...
	for blcokFrom.Uint64() <= blockMax {
		...
		go get_tx_to_target_contract_loop(source_address[:], "0x37bC7498f4FF12C19678ee8fE19d713b87F6a9e6", blcokFrom, blockTo, wg)
		// progress_bar = false
		blcokFrom = blockTo
	}
	wg.Wait()
...
func get_tx_to_target_contract_loop(source_address []string, target_contract string, blcokFrom *big.Int, blockTo *big.Int, wg *sync.WaitGroup) {
	defer wg.Done()
	...
	for blcokFrom.Uint64() <= blockTo.Uint64() {
		...
		for i, tx := range transactions {
			to_address := tx.To()
			if to_address != nil {
				if *to_address == contractAddress {
					sender, err := client.TransactionSender(context.Background(), tx, block.Hash(), uint(i))
					if err != nil {
						fmt.Println(err)
					}
					// for _, sourceAddress_str := range source_address {
					// sourceAddress := common.HexToAddress(sourceAddress_str)
					// if sender == sourceAddress {
					// if *tx.To() == contractAddress {
					tx_data_i := make([]string, len(log_headers))
					function_name, inputMap := DecodeByAbi.DecodeInput(tx, router)
					// "txHash", "blockNum", "index", "sender", "receiver", "function_name", "data",
					tx_data_i[0] = tx.Hash().Hex()
					tx_data_i[1] = blcokFrom.String()
					tx_data_i[2] = strconv.Itoa(i)
					tx_data_i[3] = sender.String()
					tx_data_i[4] = target_contract
					tx_data_i[5] = function_name
					tx_data_i[6] = fmt.Sprintf("%v", inputMap)
					log_writer.Write(tx_data_i[:])
					// }
					// }
					// }
				}
		...
		blcokFrom = big.NewInt(blcokFrom.Int64() + 1)
	}

}

func Get_multi_wallet_tx_main() {
	// wg := &sync.WaitGroup{}
	...
	log_csv := "chainlink_multisign_wallet_tx.csv"
	log_headers := []string{
		"txHash", "destination", "value", "data", "function_name", "inner_input",
	}
	log_file, log_writer, _ := CsvWriter.Get_writer(save_file_path, log_csv, log_headers)
	defer log_file.Close()
	defer log_writer.Flush()

	for i := 0; i < len(in_tx); i++ {
		method := in_tx[i][method_index]
		// if i%100 == 0 {
		// 	println(i)
		// }
		if method == "Submit Transaction" {
			txhash := in_tx[i][txhash_index]
			tx_info, _, _ := client.TransactionByHash(context.Background(), common.HexToHash(txhash))

			_, inputMap := DecodeByAbi.DecodeInput(tx_info, router)
			var data FunctionInputs
			err = mapstructure.Decode(inputMap, &data)
			if err != nil {
				log.Fatal(err)
			}
			tmp_hex_address := data.Destination.Hex()
			if tmp_hex_address == "0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419" {
				tx_data_i := make([]string, 6)

				tx_data_i[0] = txhash

				// ttt := common.Address(inputMap["destination"])
				tx_data_i[1] = data.Destination.Hex()
				tx_data_i[2] = data.Value.String()
				tx_data_i[3] = string(data.Data)

				var join_str []string
				for _, str_i := range data.Data {
					tmp_str_for_input := fmt.Sprintf("%02x", str_i)
					join_str = append(join_str, tmp_str_for_input)
				}

				joined_str := "0x" + strings.Join(join_str, "")

				// ttt2 := bytes(data.Data)
				// bs := []byte(strconv.Itoa(data.Data))
				function_name, inputMap_inner := DecodeByAbi.DecodeInputByStrHash(joined_str, inner_router)
				// 0xf8a2abd300000000000000000000000000c7a37b03690fb9f41b5c5af8131735c7275446
				// 0xf8a2abd300000000000000000000000000c7a37b369fb9f41b5c5af8131735c7275446
				// 0xf8a2abd30000000000000c7a37b369fb9f41b5c5af8131735c7275446
				tx_data_i[4] = function_name
				tx_data_i[5] = fmt.Sprintf("%v", inputMap_inner)
				// inputMap_inner = inputMap_inner
				// tx_data_i[1] = hex.EncodeToString(tx_info.Data())
				println(i)
				println(fmt.Sprintf("%v", tx_data_i))
				println("---------------------------------------------")
				log_writer.Write(tx_data_i[:])
			}
